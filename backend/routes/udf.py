...
from schemas.udf import UDF, UDFCreate, UDFUpdate, PydanticUDFGenerator
from models.udf import UDFModel
...
def create_udf(udf_create: UDFCreate, db: Session = Depends(get_db)):
    ...
    if model_class is None:
        raise HTTPException(status_code=400, detail=f"Base model '{udf_create.base_model}' not found")
...
def update_udf(udf_id: int, udf_update: UDFUpdate, db: Session = Depends(get_db)):
    ...
    if model_class is None:
        raise HTTPException(status_code=400, detail=f"Base model '{udf_update.base_model}' not found")
...
def get_pydantic_code(udf_id: int, db: Session = Depends(get_db)):
    ...
    generator = PydanticUDFGenerator(udf_json=db_udf.udf_json)
